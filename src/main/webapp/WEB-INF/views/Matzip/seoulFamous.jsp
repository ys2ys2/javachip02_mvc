                        ...
                        <div class="comment">
                            <div class="user-info">
                                <img src="${pageContext.request.contextPath}/resources/images/2.jpg" 
                                     alt="<c:out value='${comment.t_smc_author_id}'/>" 
                                     class="de">
                                <span class="username"><c:out value="${comment.t_smc_author_id}" /></span>
                            ...
                            <div class="comment-actions">
                                ...
                                <c:if test="${comment.t_smc_author_id == member.m_idx}">
...
$(function() {
    ...
    $(".comment-submit").on("click", function() {
        ...
        $.ajax({
            type: "post",
            url: "secomments", // 서버에 요청하는 URL을 secomments로 변경
            data: { 
                m_idx: m_idx, 
...
function loadComments() {
    $.ajax({
        type: "get",
        url: "secommentsList", // 서버에서 댓글 목록 가져오는 URL을 secommentsList로 변경
        dataType: "json",
...
$(".comment-delete").on("click", function(){
	const t_smc_idx = $(this).data('t_smc_idx'); // 댓글 번호 가져오기
	const t_smc_author_id = $(this).data('t_smc_author_id'); // 댓글 작성자 ID
	...
	if (confirm("정말로 이 댓글을 삭제하시겠습니까?")) {
		$.ajax({
			type: "post",
			url: "deleteComment", // URL 수정 필요 없음
			data: {
				t_smc_idx: t_smc_idx, // 댓글 번호 전송
				t_smc_author_id: t_smc_author_id // 댓글 작성자 ID 전송
			},
			success: function(response){
				if(response.trim() === "SUCCESS"){
					alert("댓글이 삭제되었습니다.");
...
$(".comment-edit").on("click", function(){
	...
	if (newCommentContent !== null && newCommentContent.trim() !== "") {
		$.ajax({
			type: "post",
			url: "editComment", // URL 수정 필요 없음
			data: {
				commentId: t_smc_idx,  // 댓글 번호 전송
				newComment: newCommentContent, // 새로운 댓글 내용
				authorId: t_smc_author_id // 댓글 작성자 ID
			},
			success: function(response){
				if(response.trim() === "SUCCESS"){
					alert("댓글이 수정되었습니다.");
